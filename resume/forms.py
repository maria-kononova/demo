...
class ResumeForm(forms.Form):
    ...
    possibility_of_transfer = forms.ChoiceField(label='Переезд', choices=POSSIBILITY_OF_TRANSFER_CHOICES,
                                                widget=forms.Select(attrs={"class": "myfield_select"}))
    business_trips = forms.ChoiceField(label='Готовность к командировкам', choices=BUSINESS_TRIPS_CHOICES,
                                       widget=forms.Select(attrs={"class": "myfield_select"}))
    desired_time_in_the_way = forms.ChoiceField(label='Желаемое время в пути', choices=DESIRED_TIME_CHOICES,
                                                widget=forms.Select(attrs={"class": "myfield_select"}))
    # На hh.ru их можно выбрать несколько...
    driving_license = forms.ChoiceField(label='Водительские права', choices=DRIVING_LISENSE_CHOICES,
                                        widget=forms.Select(attrs={"class": "myfield_select"}))
    availability_car = forms.BooleanField(label='Наличие автомобиля', required=False, initial=False,
                                          widget=forms.CheckboxInput())
    locale_resume = forms.ChoiceField(label='Локаль резюме', choices=LOCALE_RESUME_CHOICES,
                                      widget=forms.Select(attrs={"class": "myfield_select"}))
...
class AboutJobForm(forms.Form):
    ...
    # Их тоже несколько
    specialization = forms.ChoiceField(label='Специализация', choices=SPECIALIZATION_CHOICES,
                                       widget=forms.Select(attrs={"class": "myfield_select"}))
    desired_salary = forms.IntegerField(label='Желаемая зарплата', max_value=1000000,
                                        widget=forms.NumberInput(attrs={'size': '7', "class": "myfield field_salary"}))
    currency = forms.ChoiceField(label='Валюта', choices=CURRENCY_CHOICES,
                                 widget=forms.Select(attrs={"class": "myfield_select"}))
    # Их тоже несколько
    busyness = forms.ChoiceField(label='Занятость', choices=BUSYNESS_CHOICES,
                                 widget=forms.Select(attrs={"class": "myfield_select"}))
    work_timetable = forms.ChoiceField(label='График работы', choices=WORK_TIME_CHOICES,
                                       widget=forms.Select(attrs={"class": "myfield_select"}))
